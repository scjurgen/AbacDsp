...
class AudioPluginAudioProcessor : public juce::AudioProcessor, public juce::AudioProcessorValueTreeState::Listener
{
  public:
        ...
        , m_patchIndex(1, 0)
    {
        ...
        m_parameters.addParameterListener("position", this);
        m_parameters.addParameterListener("advance", this);
    ...
    {
        pluginRunner = std::make_unique<SamplePlayerTimeStretched<NumSamplesPerBlock>>(static_cast<float>(sampleRate));
...
#pragma GCC diagnostic ignored "-Wimplicit-float-conversion"
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
    {
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID("type", 1), "Sample", juce::StringArray{"Sample1", "Sample2", "Sample3", "Sample4"}, 0));
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            ...
            0));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("position", 1), "Position", juce::NormalisableRange<float>(0, 100, 0.01, 1, false), 0,
            juce::String("Position"), juce::AudioProcessorParameter::genericParameter,
            [](float value, float) { return juce::String(value, 2) + " %"; }));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("advance", 1), "Advance", juce::NormalisableRange<float>(0.1, 10, 0.01, 0.25, false), 1,
            juce::String("Advance"), juce::AudioProcessorParameter::genericParameter,
...
#pragma GCC diagnostic pop
    ...
    void parameterChanged(const juce::String& parameterID, float newValue) override
    {
        ...
        static const std::map<juce::String, std::function<void(AudioPluginAudioProcessor&, float)>> parameterMap{
            ...
            {"position", [](const AudioPluginAudioProcessor& p, const float v) { p.pluginRunner->setPosition(v); }},
            {"advance", [](const AudioPluginAudioProcessor& p, const float v) { p.pluginRunner->setAdvance(v); }},
    ...
    void loadPatchDirect(const std::vector<int>& patchIndex)
    {
        ...
        if (auto* p = m_parameters.getParameter("position"))
        {
            const auto& range = m_parameters.getParameterRange("position");
            float normalized = range.convertTo0to1(params.position);
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("advance"))
        {
            const auto& range = m_parameters.getParameterRange("advance");
            float normalized = range.convertTo0to1(params.advance);
            p->setValueNotifyingHost(normalized);
        }
    }
