...
class AudioPluginAudioProcessor : public juce::AudioProcessor, public juce::AudioProcessorValueTreeState::Listener
{
  public:
    ...
    ~AudioPluginAudioProcessor() override
    {
        ...
        m_parameters.removeParameterListener("position", this);
        m_parameters.removeParameterListener("advance", this);
    }
...
#pragma GCC diagnostic ignored "-Wimplicit-float-conversion"
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
    {
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID("type", 1), juce::String::fromUTF8("Sample"),
            juce::StringArray{juce::String::fromUTF8("Sample1"), juce::String::fromUTF8("Sample2"),
                              juce::String::fromUTF8("Sample3"), juce::String::fromUTF8("Sample4")},
            0));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("position", 1), juce::String::fromUTF8("Position"),
            juce::NormalisableRange<float>(0, 100, 0.01, 1, false), 0,
            juce::AudioParameterFloatAttributes{}.withLabel("%").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 2) + " %"; })));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("advance", 1), juce::String::fromUTF8("Advance"),
...
#pragma GCC diagnostic pop
    ...
    void applyLoadedParametersToHost()
    {
        ...
        if (auto* p = m_parameters.getParameter("position"))
        {
            ...
            float normalized = range.convertTo0to1(static_cast<float>(params.position));
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("advance"))
        {
            const auto& range = m_parameters.getParameterRange("advance");
            float normalized = range.convertTo0to1(static_cast<float>(params.advance));
            p->setValueNotifyingHost(normalized);
        }
    }
