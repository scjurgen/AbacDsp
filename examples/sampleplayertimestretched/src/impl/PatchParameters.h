...
//        "onOff", "patch", "input", "modulationDepth", "mix", "density", "threshold", "knee"});
    ...
    auto& get()
    {
        ...
        else if constexpr (ParamId == Id::position) return position;
        else if constexpr (ParamId == Id::advance) return advance;
...
break;
 case Id::position: if (!isEqual(get<Id::position>(), value)) {get<Id::position>() = value;m_modified = true;}
break;
 case Id::advance: if (!isEqual(get<Id::advance>(), value)) {get<Id::advance>() = value;m_modified = true;}
break;
