...
public:
  ...
  ~AudioPluginAudioProcessor() override {
    ...
    m_parameters.removeParameterListener("power", this);
    m_parameters.removeParameterListener("cx", this);
    m_parameters.removeParameterListener("cy", this);
    m_parameters.removeParameterListener("jx", this);
    m_parameters.removeParameterListener("jy", this);
    m_parameters.removeParameterListener("jrx", this);
    m_parameters.removeParameterListener("jry", this);
  }

  void prepareToPlay(const double sampleRate,
                     const int samplesPerBlock) override {
    pluginRunner = std::make_unique<JuliaSynth<NumSamplesPerBlock>>(
  ...
  void parameterChanged(const juce::String &parameterID,
    ...
    static const std::map<
        ...
        parameterMap{
            ...
            {"power", [](const AudioPluginAudioProcessor &p,
                         const float v) { p.pluginRunner->setPower(v); }},
            {"cx", [](const AudioPluginAudioProcessor &p,
                      const float v) { p.pluginRunner->setCx(v); }},
            {"cy", [](const AudioPluginAudioProcessor &p,
                      const float v) { p.pluginRunner->setCy(v); }},
            {"jx", [](const AudioPluginAudioProcessor &p,
                      const float v) { p.pluginRunner->setJx(v); }},
            {"jy", [](const AudioPluginAudioProcessor &p,
                      const float v) { p.pluginRunner->setJy(v); }},
            {"jrx", [](const AudioPluginAudioProcessor &p,
                       const float v) { p.pluginRunner->setJrx(v); }},
            {"jry", [](const AudioPluginAudioProcessor &p,
                       const float v) { p.pluginRunner->setJry(v); }},
