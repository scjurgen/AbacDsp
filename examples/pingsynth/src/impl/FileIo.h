...
class FileIo
{
  ...
  private:
...
#endif
        const auto dir = base.getChildFile("AbacDsp").getChildFile("Pingsynth");
        dir.createDirectory();
        return dir;
    }
    ...
    static juce::File getScriptDirectory()
    {
        ...
        base = base.getChildFile("Application Support");
#endif
        const auto dir = base.getChildFile("AbacDsp").getChildFile("Pingsynth").getChildFile("Scripts");
        dir.createDirectory();
        return dir;
    }
    ...
    static juce::File getLibraryDirectory()
    {
        ...
        base = base.getChildFile("Application Support");
#endif
        const auto dir = base.getChildFile("AbacDsp").getChildFile("Pingsynth").getChildFile("Library");
        dir.createDirectory();
        return dir;
    }

    // Refreshes Library/Base/ from the repo's base-scripts/ directory (only available in a
    // dev build from a real checkout - PINGSYNTH_BASE_SCRIPTS_DIR is undefined
    // otherwise, in which case this is a no-op and whatever is already on disk is used).
    static void syncBaseLibraryScripts()
    {
#ifdef PINGSYNTH_BASE_SCRIPTS_DIR
        const juce::File repoDir(PINGSYNTH_BASE_SCRIPTS_DIR);
    ...
    // in a dev build from a real checkout - PINGSYNTH_FACTORY_PATCHES_DIR is undefined
    // otherwise, in which case this is a no-op and whatever is already on disk is used).
    static void syncFactoryPatches()
    {
#ifdef PINGSYNTH_FACTORY_PATCHES_DIR
        const juce::File repoDir(PINGSYNTH_FACTORY_PATCHES_DIR);
        if (!repoDir.isDirectory())
        {
            return;
        }
