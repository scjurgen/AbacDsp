...
class MultichannelInterpolation
{
  public:
    ...
    // 6-point, 5th-order B-spline interpolation (65x)
    static void bspline_65x(const float* interleaved, float* out, float x)
    {
        static_assert(NumChannels > 0, "Need at least one channel");

        for (size_t ch = 0; ch < NumChannels; ++ch)
        {
            const auto y0 = interleaved[NumChannels * 0 + ch];
            const auto y1 = interleaved[NumChannels * 1 + ch];
            const auto y2 = interleaved[NumChannels * 2 + ch];
            const auto y3 = interleaved[NumChannels * 3 + ch];
            const auto y4 = interleaved[NumChannels * 4 + ch];
            const auto y5 = interleaved[NumChannels * 5 + ch];

            const auto ym2py2 = y0 + y4; // y[-2] + y[+2]
            const auto ym1py1 = y1 + y3; // y[-1] + y[+1]
            const auto y2mym2 = y4 - y0; // y[+2] - y[-2]
            const auto y1mym1 = y3 - y1; // y[+1] - y[-1]
            const auto sixthym1py1 = 1.f / 6.f * ym1py1;

            const auto c0 = 1.f / 120.f * ym2py2 + 13.f / 60.f * ym1py1 + 11.f / 20.f * y2;
            const auto c1 = 1.f / 24.f * y2mym2 + 5.f / 12.f * y1mym1;
            const auto c2 = 1.f / 12.f * ym2py2 + sixthym1py1 - 0.5f * y2;
            const auto c3 = 1.f / 12.f * y2mym2 - 1.f / 6.f * y1mym1;
            const auto c4 = 1.f / 24.f * ym2py2 - sixthym1py1 + 0.25f * y2;
            const auto c5 = 1.f / 120.f * (y5 - y0) + 1.f / 24.f * (y1 - y4) + 1.f / 12.f * (y3 - y2);

            out[ch] = ((((c5 * x + c4) * x + c3) * x + c2) * x + c1) * x + c0;
        }
    }
...
struct BSpline4Kernel
{
    ...
    static float eval(const float* y, float frac) noexcept
    {
        return Interpolation<float>::bspline_43x(y, frac);
    }
};

struct BSpline6Kernel
{
    ...
    static float eval(const float* y, float frac) noexcept
    {
        return Interpolation<float>::bspline_65x(y, frac);
    }
};
