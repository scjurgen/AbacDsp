...
// Hand-written, morphexsynth-only glue between LuaControlArea (a shared, generic template)
// and MorphexsynthScriptEngine's types. Not in CPP_SOURCE_FILES_FIXED, so regeneration never
// touches it.
...
[[nodiscard]] inline std::vector<LuaControlDescriptor> toLuaControlDescriptors(
    const MorphexsynthScriptEngine::UiParamSlots& slots)
{
