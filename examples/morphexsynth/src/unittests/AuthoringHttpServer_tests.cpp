...
TEST_F(AuthoringHttpServerTest, PostRecordStartDispatchesToWiredCallback)
{
    server.startRecording = [] { return AuthoringRecordStartResult{true, "/tmp/abacdsp/testmodule/rec.wav", ""}; };
    ASSERT_TRUE(server.start("testmodule"));
    ...
    const auto body = runWhilePumpingMessages(
        [&]
        {
            httplib::Client client("127.0.0.1", server.boundPort());
            client.set_bearer_token_auth(server.token());
            const auto res = client.Post("/record/start", "", "application/json");
            return res ? res->body : std::string{};
        });
    const auto result = nlohmann::json::parse(body);
    EXPECT_TRUE(result.at("started").get<bool>());
    EXPECT_EQ(result.at("path").get<std::string>(), "/tmp/abacdsp/testmodule/rec.wav");
}

TEST_F(AuthoringHttpServerTest, PostRecordStopDispatchesToWiredCallback)
{
    server.stopRecording = []
    { return AuthoringRecordStopResult{true, "/tmp/abacdsp/testmodule/rec.wav", 4.0, false}; };
    ASSERT_TRUE(server.start("testmodule"));
    ...
    const auto body = runWhilePumpingMessages(
        [&]
        {
            httplib::Client client("127.0.0.1", server.boundPort());
            client.set_bearer_token_auth(server.token());
            const auto res = client.Post("/record/stop", "", "application/json");
            return res ? res->body : std::string{};
        });
    const auto result = nlohmann::json::parse(body);
    EXPECT_TRUE(result.at("wasRecording").get<bool>());
    EXPECT_DOUBLE_EQ(result.at("durationSeconds").get<double>(), 4.0);
    EXPECT_FALSE(result.at("capped").get<bool>());
}
