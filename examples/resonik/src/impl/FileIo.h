...
class FileIo
{
  ...
  private:
    ...
    // in a dev build from a real checkout - RESONIK_FACTORY_PATCHES_DIR is undefined
    // otherwise, in which case this is a no-op and whatever is already on disk is used).
    static void syncFactoryPatches()
    {
#ifdef RESONIK_FACTORY_PATCHES_DIR
        const juce::File repoDir(RESONIK_FACTORY_PATCHES_DIR);
        if (!repoDir.isDirectory())
        {
            return;
        }
