...
// Hand-written, resonik-only glue between LuaControlArea (a shared, generic template)
// and ResonikScriptEngine's types. Not in CPP_SOURCE_FILES_FIXED, so regeneration never
// touches it.
...
[[nodiscard]] inline std::vector<LuaControlDescriptor> toLuaControlDescriptors(
    const ResonikScriptEngine::UiParamSlots& slots)
{
