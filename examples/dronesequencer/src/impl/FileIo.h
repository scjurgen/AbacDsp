...
class FileIo
{
  ...
  private:
    ...
    // in a dev build from a real checkout - DRONESEQUENCER_FACTORY_PATCHES_DIR is undefined
    // otherwise, in which case this is a no-op and whatever is already on disk is used).
    static void syncFactoryPatches()
    {
#ifdef DRONESEQUENCER_FACTORY_PATCHES_DIR
        const juce::File repoDir(DRONESEQUENCER_FACTORY_PATCHES_DIR);
        if (!repoDir.isDirectory())
        {
            return;
        }
