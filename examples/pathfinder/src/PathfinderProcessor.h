...
class AudioPluginAudioProcessor : public juce::AudioProcessor, public juce::AudioProcessorValueTreeState::Listener
{
  public:
    ...
    {
        pluginRunner = std::make_unique<PathfinderImpl<NumSamplesPerBlock>>(RateNormalizer::kInternalSampleRate);
        ...
        if (m_fileIo.currentScript().empty())
        {
            m_fileIo.updateScript(pluginRunner->scriptSkeleton());
        }
...
#pragma GCC diagnostic pop
    ...
    [[nodiscard]] PathfinderKnobSlots getLuaUiParamSlots() const
    {
        return pluginRunner ? pluginRunner->uiParamSlots() : PathfinderKnobSlots{};
    }
    [[nodiscard]] bool collectRetiredGraphs() const noexcept
    {
        return pluginRunner && pluginRunner->collectRetired();
    }
  ...
  private:
    ...
    std::unique_ptr<PathfinderImpl<NumSamplesPerBlock>> pluginRunner;
