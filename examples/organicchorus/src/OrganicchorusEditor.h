...
#include "OrganicchorusProcessor.h"
    ...
    void resized() override
    {
        ...
        else
        {
...

            {
                ...
                box.items.add(juce::FlexItem(configurationDrop)
                                  .withFlex(0)
                                  .withWidth(300)
            ...
            }
            {
                ...
                box.flexDirection = juce::FlexBox::Direction::row;
                box.justifyContent = juce::FlexBox::JustifyContent::spaceAround;
                box.items.add(juce::FlexItem(toneDial).withFlex(1).withMargin(knobMarginSmall));
...
#pragma GCC diagnostic pop
    ...
    void initWidgets()
    {
        addAndMakeVisible(configurationDrop);
        configurationDrop.addItemList(valueTreeState.getParameter("configuration")->getAllValueStrings(), 1);
        configurationDropAttachment = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(
            valueTreeState, "configuration", configurationDrop);
        configurationDrop.setTooltip(juce::String::fromUTF8("Configuration (Classic, Wide, Tri Ensemble, Flanger)"));
        addAndMakeVisible(toneDial);
        toneDial.reset(valueTreeState, "tone");
        toneDial.setLabelText(juce::String::fromUTF8("Tone"));
        toneDial.setTooltip(juce::String::fromUTF8("Tone (0 to 100)"));
        addAndMakeVisible(speedDial);
        speedDial.reset(valueTreeState, "speed");
        speedDial.setLabelText(juce::String::fromUTF8("Speed"));
        speedDial.setTooltip(juce::String::fromUTF8("Speed (0.05 to 6.0 Hz)"));
        ...
        tapeSpeedDial.setTooltip(juce::String::fromUTF8("Tape Speed (-24 to 24 st)"));
        ...
        mixDial.setTooltip(juce::String::fromUTF8("Mix (0 to 100 %)"));
    ...
    void switchPage(Page page)
    {
        ...
        {
            configurationDrop.setVisible(true);
            toneDial.setVisible(true);
    ...
    void showAboutDialog()
    {
        ...
        const juce::String body =
            ...
            "\n\n"
            "Organic, multi-voice BBD-style chorus/flanger built on a genuine variable-write-clock delay transport per "
            "voice, driven by mean-reverting drift and flutter so the ensemble never quite repeats.\n\nPart of the "
...
#ifdef ORGANICCHORUS_SCRIPTING_DOCS_FILE
        editorComponent->onOpenDocs = []
        { juce::URL(juce::File(ORGANICCHORUS_SCRIPTING_DOCS_FILE)).launchInDefaultBrowser(); };
  ...
  private:
    ...
    juce::ComboBox configurationDrop{};
    std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> configurationDropAttachment;
    CustomRotaryDial toneDial{this};
