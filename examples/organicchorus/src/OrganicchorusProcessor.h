...
class AudioPluginAudioProcessor : public juce::AudioProcessor, public juce::AudioProcessorValueTreeState::Listener
{
  public:
    ...
    AudioPluginAudioProcessor()
        ...
        , m_patchIndex(0, 0)
    {
        m_parameters.addParameterListener("configuration", this);
        m_parameters.addParameterListener("tone", this);
    ...
    {
        m_parameters.removeParameterListener("configuration", this);
        m_parameters.removeParameterListener("tone", this);
    ...
    {
        pluginRunner = std::make_unique<OrganicChorusImpl<NumSamplesPerBlock>>(RateNormalizer::kInternalSampleRate);
...
#pragma GCC diagnostic ignored "-Wimplicit-float-conversion"
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
    {
        ...
        params.push_back(std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID("configuration", 1), juce::String::fromUTF8("Configuration"),
            juce::StringArray{juce::String::fromUTF8("Classic"), juce::String::fromUTF8("Wide"),
                              juce::String::fromUTF8("Tri Ensemble"), juce::String::fromUTF8("Flanger")},
            0));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("tone", 1), juce::String::fromUTF8("Tone"),
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            ...
            juce::NormalisableRange<float>(0.05, 6.0, 0.01, 0.35, false), 0.8,
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            ...
            juce::NormalisableRange<float>(-24, 24, 0.1, 1, true), 0,
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            ...
            juce::NormalisableRange<float>(-100, 100, 0.1, 1, true), 0,
            juce::AudioParameterFloatAttributes{}.withLabel("%").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 0) + " %"; })));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
...
#pragma GCC diagnostic pop
    ...
    void parameterChanged(const juce::String& parameterID, float newValue) override
    {
        ...
        static const std::map<juce::String, std::function<void(AudioPluginAudioProcessor&, float)>> parameterMap{
            {"configuration",
             ...
             {
                 p.pluginRunner->setConfiguration(static_cast<int>(v));
                 p.m_fileIo.updateParameter(PatchParameters::Id::configuration, v);
             }},
            {"tone",
             [](AudioPluginAudioProcessor& p, const float v)
             {
                 p.pluginRunner->setTone(v);
                 p.m_fileIo.updateParameter(PatchParameters::Id::tone, v);
             }},
            {"speed",
             [](AudioPluginAudioProcessor& p, const float v)
             {
    ...
    void applyLoadedParametersToHost()
    {
        ...
        if (auto* p = m_parameters.getParameter("configuration"))
        {
            const auto& range = m_parameters.getParameterRange("configuration");
            float normalized = range.convertTo0to1(static_cast<float>(params.configuration));
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("tone"))
        {
            const auto& range = m_parameters.getParameterRange("tone");
            float normalized = range.convertTo0to1(static_cast<float>(params.tone));
            p->setValueNotifyingHost(normalized);
        }
...
#pragma GCC diagnostic pop
    ...
    [[nodiscard]] OrganicChorusScriptEngine::UiParamSlots getLuaUiParamSlots() const
    {
        return pluginRunner ? pluginRunner->uiParamSlots() : OrganicChorusScriptEngine::UiParamSlots{};
    }
  ...
  private:
    ...
    std::unique_ptr<OrganicChorusImpl<NumSamplesPerBlock>> pluginRunner;
