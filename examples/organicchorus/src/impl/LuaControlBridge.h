...
// Hand-written, organicchorus-only glue between LuaControlArea (a shared, generic
// template) and OrganicChorusScriptEngine's types. Not in CPP_SOURCE_FILES_FIXED, so
...
[[nodiscard]] inline std::vector<LuaControlDescriptor> toLuaControlDescriptors(
    const OrganicChorusScriptEngine::UiParamSlots& slots)
{
