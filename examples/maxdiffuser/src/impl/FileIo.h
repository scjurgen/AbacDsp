...
public:
  ...
  bool handlePatchChange(const std::vector<int> &newPatchIndex,
    ...
    if (m_currentParams.isModified()) {
      const std::string message = "Parameters have changed, do you want to "
                                  "save before loading new patch?";
  ...
  [[nodiscard]] std::vector<std::string> listPatchNames() const {
    ...
    for (const auto &f : getPatchDirectory().findChildFiles(
  ...
  bool savePatchNamed(const std::string &name) {
    ...
    if (!out) {
      std::cerr << "FileIo: ERROR - Failed to open " << filename
...
private:
...
#endif
    const auto dir = base.getChildFile("AbacDsp").getChildFile("Maxdiffuser");
    dir.createDirectory();
    return dir;
  }
  ...
  // Named patches are stored one file per name, so the name has to survive as a
  // filename; strip characters that are invalid (or awkward, e.g. path
  // separators) across platforms.
  static std::string getNamedPatchFilename(const std::string &name) {
    const juce::String sanitized =
        juce::String(name).removeCharacters("/\\:*?\"<>|").trim();
    if (sanitized.isEmpty()) {
      return {};
    }
    return getPatchDirectory()
        .getChildFile(sanitized + ".json")
        .getFullPathName()
        .toStdString();
  }
  ...
  bool loadPatch(const std::vector<int> &patchIndex) {
    ...
    std::ifstream in(filename);
    if (!in) {
      std::cout << "FileIo: File " << filename << " not found, using defaults"
                << std::endl;
