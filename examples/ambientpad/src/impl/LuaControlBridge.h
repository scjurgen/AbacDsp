...
// Hand-written, ambientpad-only glue between LuaControlArea (a shared, generic template)
// and AmbientPadScriptEngine's types. Not in CPP_SOURCE_FILES_FIXED, so regeneration never
// touches it.
...
[[nodiscard]] inline std::vector<LuaControlDescriptor> toLuaControlDescriptors(
    const AmbientPadScriptEngine::UiParamSlots& slots)
{
