...
class AudioPluginAudioProcessor : public juce::AudioProcessor, public juce::AudioProcessorValueTreeState::Listener
{
  public:
    ...
    {
        pluginRunner = std::make_unique<AmbientPadImpl<NumSamplesPerBlock>>(RateNormalizer::kInternalSampleRate);
...
#pragma GCC diagnostic ignored "-Wimplicit-float-conversion"
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
    {
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            ...
            juce::NormalisableRange<float>(-60, 12, 0.1, 1, false), -30,
            juce::AudioParameterFloatAttributes{}.withLabel("dB").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 1) + " dB"; })));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("note", 1), juce::String::fromUTF8("Note"),
            juce::NormalisableRange<float>(0, 127, 1, 1, false), 69,
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("material", 1), juce::String::fromUTF8("Material"),
        ...
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            ...
            juce::AudioParameterFloatAttributes{}.withLabel("").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 2) + " "; })));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("motion", 1), juce::String::fromUTF8("Motion"),
            juce::NormalisableRange<float>(0, 1, 0.01, 1, false), 0.3,
            juce::AudioParameterFloatAttributes{}.withLabel("").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 2) + " "; })));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("breath", 1), juce::String::fromUTF8("Breath"),
            juce::NormalisableRange<float>(0, 1, 0.01, 1, false), 0.3,
            juce::AudioParameterFloatAttributes{}.withLabel("").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 2) + " "; })));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("stability", 1), juce::String::fromUTF8("Stability"),
            juce::NormalisableRange<float>(0, 1, 0.01, 1, false), 0.5,
            juce::AudioParameterFloatAttributes{}.withLabel("").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 2) + " "; })));
        params.push_back(std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID("bloom", 1), juce::String::fromUTF8("Bloom"),
            juce::NormalisableRange<float>(0, 1, 0.01, 1, false), 0.3,
            juce::AudioParameterFloatAttributes{}.withLabel("").withStringFromValueFunction(
                [](float value, int) { return juce::String(value, 2) + " "; })));
...
#pragma GCC diagnostic pop
    ...
    void parameterChanged(const juce::String& parameterID, float newValue) override
    {
        ...
        static const std::map<juce::String, std::function<void(AudioPluginAudioProcessor&, float)>> parameterMap{
            ...
            {"note",
             [](AudioPluginAudioProcessor& p, const float v)
             {
                 p.pluginRunner->setNote(v);
                 p.m_fileIo.updateParameter(PatchParameters::Id::note, v);
             }},
            {"play",
             [](AudioPluginAudioProcessor& p, const float v)
             {
            ...
            {"light",
             [](AudioPluginAudioProcessor& p, const float v)
             {
                 p.pluginRunner->setLight(v);
                 p.m_fileIo.updateParameter(PatchParameters::Id::light, v);
             }},
            {"motion",
             [](AudioPluginAudioProcessor& p, const float v)
             {
                 p.pluginRunner->setMotion(v);
                 p.m_fileIo.updateParameter(PatchParameters::Id::motion, v);
             }},
            {"breath",
             [](AudioPluginAudioProcessor& p, const float v)
             {
                 p.pluginRunner->setBreath(v);
                 p.m_fileIo.updateParameter(PatchParameters::Id::breath, v);
             }},
            {"stability",
             [](AudioPluginAudioProcessor& p, const float v)
             {
                 p.pluginRunner->setStability(v);
                 p.m_fileIo.updateParameter(PatchParameters::Id::stability, v);
             }},
            {"bloom",
             [](AudioPluginAudioProcessor& p, const float v)
             {
                 p.pluginRunner->setBloom(v);
                 p.m_fileIo.updateParameter(PatchParameters::Id::bloom, v);
             }},
            {"hold",
             [](AudioPluginAudioProcessor& p, const float v)
             {
    ...
    void applyLoadedParametersToHost()
    {
        ...
        if (auto* p = m_parameters.getParameter("note"))
        {
            const auto& range = m_parameters.getParameterRange("note");
            float normalized = range.convertTo0to1(static_cast<float>(params.note));
            p->setValueNotifyingHost(normalized);
        }
        ...
        if (auto* p = m_parameters.getParameter("material"))
        {
        ...
        {
            const auto& range = m_parameters.getParameterRange("light");
            float normalized = range.convertTo0to1(static_cast<float>(params.light));
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("motion"))
        {
            const auto& range = m_parameters.getParameterRange("motion");
            float normalized = range.convertTo0to1(static_cast<float>(params.motion));
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("breath"))
        {
            const auto& range = m_parameters.getParameterRange("breath");
            float normalized = range.convertTo0to1(static_cast<float>(params.breath));
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("stability"))
        {
            const auto& range = m_parameters.getParameterRange("stability");
            float normalized = range.convertTo0to1(static_cast<float>(params.stability));
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("bloom"))
        {
            const auto& range = m_parameters.getParameterRange("bloom");
            float normalized = range.convertTo0to1(static_cast<float>(params.bloom));
            p->setValueNotifyingHost(normalized);
        }
        if (auto* p = m_parameters.getParameter("hold"))
        {
...
#pragma GCC diagnostic pop
    ...
    [[nodiscard]] AmbientPadScriptEngine::UiParamSlots getLuaUiParamSlots() const
    {
  ...
  private:
    ...
    std::unique_ptr<AmbientPadImpl<NumSamplesPerBlock>> pluginRunner;
