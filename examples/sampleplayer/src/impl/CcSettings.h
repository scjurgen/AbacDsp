...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(CcMappingOverride, paramId,
                                                controller, valueLow, valueHigh)
...
public:
  ...
  static void save(const std::vector<CcMappingOverride> &overrides) {
    ...
    if (!out) {
      std::cerr << "CcSettings: ERROR - Failed to open " << getFilename()
