...
//        "onOff", "patch", "input", "modulationDepth", "mix", "density", "threshold", "knee"});
    ...
    auto& get()
    {
        ...
        else if constexpr (ParamId == Id::filterType) return filterType;
...
break;
 case Id::filterType: if (!isEqual(get<Id::filterType>(), value)) {get<Id::filterType>() = static_cast<size_t>(value) ;m_modified = true;}
break;
