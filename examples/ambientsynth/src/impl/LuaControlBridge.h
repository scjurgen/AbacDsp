...
// Hand-written, ambientsynth-only glue between LuaControlArea (a shared, generic template)
// and AmbientSynthScriptEngine's types. Not in CPP_SOURCE_FILES_FIXED, so regeneration never
// touches it.
...
[[nodiscard]] inline std::vector<LuaControlDescriptor> toLuaControlDescriptors(
    const AmbientSynthScriptEngine::UiParamSlots& slots)
{
