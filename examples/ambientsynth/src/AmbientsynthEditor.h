...
#include "AmbientsynthProcessor.h"
    ...
    void resized() override
    {
        ...
        if (m_currentPage == Page::Performance)
        {
...

            {
                ...
                box.items.add(juce::FlexItem(cutoffDial).withFlex(1).withMargin(knobMarginSmall));
                box.items.add(juce::FlexItem(resonanceDial).withFlex(1).withMargin(knobMarginSmall));
                box.items.add(juce::FlexItem(filterTypeDrop)
                                  .withFlex(1)
        ...
        else
        {
...

            {
                ...
                box.items.add(juce::FlexItem(levelMeterGauge).withHeight(300).withMargin(knobMarginSmall));
...
#pragma GCC diagnostic pop
    ...
    void timerCallback() override
    {
        if (processorRef.hasRunner())
        {
            ...
            levelMeterGauge.update(processorRef.getInputDbLoad(), processorRef.getOutputDbLoad());
    ...
    void initWidgets()
    {
        ...
        levelDial.setTooltip(juce::String::fromUTF8("Level (-60 to 12 dB)"));
        addAndMakeVisible(materialDial);
        ...
        cutoffDial.reset(valueTreeState, "cutoff");
        cutoffDial.setLabelText(juce::String::fromUTF8("Cutoff"));
        cutoffDial.setTooltip(juce::String::fromUTF8("Cutoff (0 to 1)"));
        addAndMakeVisible(resonanceDial);
        resonanceDial.reset(valueTreeState, "resonance");
        resonanceDial.setLabelText(juce::String::fromUTF8("Resonance"));
        resonanceDial.setTooltip(juce::String::fromUTF8("Resonance (0 to 1)"));
        addAndMakeVisible(filterTypeDrop);
        filterTypeDrop.addItemList(valueTreeState.getParameter("filterType")->getAllValueStrings(), 1);
        filterTypeDropAttachment = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(
            valueTreeState, "filterType", filterTypeDrop);
        filterTypeDrop.setTooltip(juce::String::fromUTF8("Filter (LP4, LP2, LP1Notch, Notch, BP2, HP1LP3, AP4)"));
        addAndMakeVisible(bloomDial);
        bloomDial.reset(valueTreeState, "bloom");
        bloomDial.setLabelText(juce::String::fromUTF8("Bloom"));
        bloomDial.setTooltip(juce::String::fromUTF8("Bloom (0 to 1)"));
        ...
        addAndMakeVisible(levelMeterGauge);
        levelMeterGauge.setLabelText(juce::String::fromUTF8("Level"));
        levelMeterGauge.setTooltip(juce::String::fromUTF8("Level (0 to 100 %)"));
    ...
    void switchPage(Page page)
    {
        ...
        if (page == Page::Performance)
        {
            ...
            filterTypeDrop.setVisible(true);
    ...
    void showAboutDialog()
    {
        ...
        const juce::String body =
            ...
            "\n\n"
            "Ambient synth voice for real-time performance: two morphing wavetable layers through a discrete-type "
            "pole-mixing filter, each of Cutoff/Resonance/Material/Breath/Pitch carrying its own Ornstein-Uhlenbeck "
            "wander plus an independent LFO. Played from a MIDI keyboard - white keys C4..A4 pick a harmonic region, "
            "notes below C4 set the bass pedal - or scripted via Lua.\n\nPart of the AbacDsp project - core DSP "
...
#ifdef AMBIENTSYNTH_SCRIPTING_DOCS_FILE
        editorComponent->onOpenDocs = []
        { juce::URL(juce::File(AMBIENTSYNTH_SCRIPTING_DOCS_FILE)).launchInDefaultBrowser(); };
  ...
  private:
    ...
    juce::ComboBox filterTypeDrop{};
    std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> filterTypeDropAttachment;
