...
class FileIo
{
  ...
  private:
...
#endif
        const auto dir = base.getChildFile("AbacDsp").getChildFile("Tapelooper");
        dir.createDirectory();
        return dir;
    }
    ...
    // in a dev build from a real checkout - TAPELOOPER_FACTORY_PATCHES_DIR is undefined
    // otherwise, in which case this is a no-op and whatever is already on disk is used).
    static void syncFactoryPatches()
    {
#ifdef TAPELOOPER_FACTORY_PATCHES_DIR
        const juce::File repoDir(TAPELOOPER_FACTORY_PATCHES_DIR);
        if (!repoDir.isDirectory())
        {
            return;
        }
