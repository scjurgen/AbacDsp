...
The Lua API available to every script, shared by all Lua-scripted examples generated via
`JuceStandaloneGenerator`. For how the engine dispatches to a loaded script internally
(compile/apply flow, realtime handler dispatch), see `LUA.md`; for what's specific to one
example (its own `SetXxx()` calls, note-generation hooks, etc.), see that example's own
README.md "Scripting" section.
