...
size_t solo{0};
float vol{0.0f};
float reverbLevelWet{-120.0f};
float reverbDecay{1000.0f};
...
float vol9{-12.0f};
float vol10{-12.0f};
float revFeed1{-100.0f};
float revFeed2{-100.0f};
float revFeed3{-100.0f};
float revFeed4{-100.0f};
float revFeed5{-100.0f};
float revFeed6{-100.0f};
float revFeed7{-100.0f};
float revFeed8{-100.0f};
float revFeed9{-100.0f};
float revFeed10{-100.0f};
float pitch1{0.0f};
float pitch2{0.0f};
float pitch3{0.0f};
float pitch4{0.0f};
...
//        "onOff", "patch", "input", "modulationDepth", "mix", "density", "threshold", "knee"});
    ...
    auto& get()
    {
        if constexpr (ParamId == Id::type) return type;
        else if constexpr (ParamId == Id::subset) return subset;
...
break;
 case Id::subset: if (!isEqual(get<Id::subset>(), value)) {get<Id::subset>() = static_cast<size_t>(value) ;m_modified = true;}
break;
