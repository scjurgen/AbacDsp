...
displayed value changes. Spectrogram/CircularBar/SliceWave, and the maxdiffuser-style
hand-written `customtype` gauges (Bands/Sizes), are **Strategy E** (intentionally decorative,
`setAccessible(false)`) - per user decision, the information these carry is aesthetic, not
safety- or task-relevant, so no synthetic accessible value is invented for them. Only the
VU/level and CPU gauges get Strategy C.
...
`CpuGauge`/`WaveformGauge` component, exposing the current value on demand. This runs on the
message thread only (the timer-poll -> `update()` call chain already does), so it introduces
no audio-thread work. Do **not** fire `notifyAccessibilityEvent(valueChanged)` from every
`update()` call (that runs at `TimerHertz`, i.e. continuously while audio plays) - see
Finding 15 for why that would be actively harmful, and for the edge-triggered alert mechanism
that replaces it for the safety-relevant transitions (clipping, CPU overload).
...
**Finding 15 - VU/CPU/waveform value exposure needs event-based diagnostics, not raw
continuous live-region announcements (extends Finding 4).**
File/class/method: `inc/GenericMeter.h`, `inc/CpuMeter.h`, `inc/WaveformMeter.h` - same
classes as Finding 4; also a new shared component, e.g. `DiagnosticsLog`, plus its menu-bar
entry point in `{Module}Editor.h`.
Problem: Finding 4's mechanism as originally specified - fire
`notifyAccessibilityEvent(valueChanged)` from every `update()` call, driven by a
`TimerHertz` (60 Hz) timer - would announce a new value up to 60 times per second while
audio plays. Screen readers treat `valueChanged` as an interruption-worthy event by default;
continuous per-frame firing produces unstoppable chatter the instant audio starts. This is a
known screen-reader UX anti-pattern (a constantly-changing live region), not a hypothetical -
it would very likely be worse for a screen reader user than today's silence.
Affected users/failure mode: a screen reader user starts playback expecting occasional,
meaningful feedback (a clip warning, a CPU overload) and instead gets a continuous numeric
readout drowning out all other announcements (menu navigation, status-bar messages) for as
long as audio plays.
Required behavior: separate the two access patterns a sighted user gets "for free" from a
glance at the meter, but a screen reader user needs deliberately designed:
1. **On-demand current value** - a queryable control/action reporting e.g. "Input -6.2 dB,
   Output -4.8 dB, CPU 12%" only when the user asks (Tab to it, or a dedicated "read levels"
   action) - matching how a sighted user glances at the meter only when they choose to.
2. **Unsolicited alerts for state changes that need attention** - clipping started/stopped,
   CPU load crossing a high-water mark, sustained output silence (possible signal-path
   problem) - announced once per state transition (edge-triggered, with hysteresis so a
   value oscillating around a threshold does not re-announce every frame), not on every poll.
Recommended mechanism: keep Finding 4's `AccessibilityHandler`/value-interface addition for
the on-demand path (part 1) - a value interface is read on request, not pushed, so no change
needed there. For part 2, add a small `DiagnosticsLog` component (Strategy C, role
`AccessibilityRole::staticText`, live region, modeled on `StatusBar`'s existing live-region
pattern from Finding 5), reachable from the menu bar or a dedicated shortcut, listing recent
threshold-crossing events with timestamps - so a user who was mid-action when an alert fired
can review what they missed, not only the latest one. Each gauge's existing `update()` call
site compares the new value against a per-metric threshold/hysteresis band and pushes a log
entry + fires `notifyAccessibilityEvent` only on a transition, never on every poll.
Scope/dependencies: extends Phase 2's Finding 4 work in the same three header files; adds one
new shared component and its menu-bar entry point. Default threshold/hysteresis values (clip
level, CPU high-water mark, silence duration) need deciding - see Open Questions.
Test: with VoiceOver running, drive a signal into clipping and back out - confirm exactly one
"clipping started" and one "clipping stopped" announcement, not a stream; separately, Tab to
a gauge and confirm an on-demand read reports the current value without needing an alert to
have fired first.
...
**Finding 16 - No dedicated high-contrast/quick-access pane; every page mode uses the same
decorative, glance-optimized styling.**
Files/classes: `{Module}Editor.h` (existing `/*START_PERFORMANCEPAGE*/` page-switch
mechanism, Section 2.3), `codegen_widgets.py`'s `create_page_switch_methods()`,
`parseboxstructure.py`, `inc/LookAndFeel.h` (`GuiLookAndFeel`), blueprint JSON schema.
Problem: every existing page mode (main layout, and the optional Performance/Settings split)
renders the same custom-painted `CustomRotaryDial` knobs, gauges, and theme skin, which are
tuned for visual/aesthetic appeal (Section 2.4's `GuiLookAndFeel` inventory) at the cost of
small hit targets, decorative chrome a screen reader has to tab past, and no always-visible
textual readout. This is a reasonable tradeoff for sighted users but is a pure tax for a
screen-reader or low-vision user who wants to change a handful of parameters quickly - there
is no mode that optimizes for that instead.
Affected users/failure mode: screen-reader and low-vision users pay the same navigation and
rendering cost as sighted users for every interaction, with none of the payoff (they cannot
see the decorative painting), even after Findings 1-8/15 close the pure accessible-name/
value/announcement gaps.
Required behavior: an opt-in third page mode ("Accessibility" pane) presenting the same
blueprint parameters through plain, high-contrast, large-target controls with always-visible
value readouts, laid out as a single reading-order list rather than a hand-authored visual
grid - optimized for fast, low-friction control access rather than appearance.
Recommended mechanism: generalize the existing Performance/Settings page-switch machinery
(already N=2 pages) to support a third, auto-generated page built directly from the
`ports-control` list in declaration/focus order (Finding 8) - no hand-authored layout grid
needed, since a plain list needs no visual composition. Style it with a separate, minimal
`LookAndFeel` variant (stock-shaped controls, no custom knob painting) using a high-contrast
palette drawn from the existing Orbit theme's OKLCH contrast validation work (per project
memory - reuse validated contrast ratios, don't invent new ones). Gate the whole page behind
an opt-in blueprint flag (e.g. `"accessibility-page": true`) so it's additive, not forced on
every blueprint at once.
Scope/dependencies: `codegen_widgets.py` (new page-generation function reusing
`create_init_widgets()`'s existing attachment logic against a second `Component`), a new
shared `inc/AccessibilityLookAndFeel.h`, `{Module}Editor.h`'s page-switch codegen splice,
one blueprint schema field. Larger than most findings here - it is new capability, not a gap
fix - but low regression risk to existing pages since it is purely opt-in and additive.
Test: keyboard-only and VoiceOver walkthrough of the accessibility pane on a canary blueprint
with `"accessibility-page": true` set; confirm every parameter is reachable in one flat Tab
sequence, every readout is visible without a focus/hover query, and contrast ratios pass the
same WCAG check already used for the Orbit theme suite.

**Finding 17 - Blueprint sections (`A1=(...)`, `A2=(...)`, ... and existing divider labels)
carry no accessibility grouping semantics.**
Files/methods: `JuceStandaloneGenerator/parseboxstructure.py` (`parse_box_structure()`/
`construct_boxes()` - the `A<n>=(...)` area syntax that already groups every blueprint's
controls into named FlexBox sections), `codegen_widgets.py`'s `create_init_widgets()`.
Precedent already in the codebase: `looper.json` places a purely decorative `"label"`-type
control (`"—— Overdub ——"`, `"—— Sequencer ——"`) as the first item of its section's area,
i.e. blueprint authors already informally mark section boundaries this way.
Problem: `construct_boxes()` uses area membership (which `short` symbols belong to `A1`,
`A2`, ...) only to build FlexBox geometry (Section 2.4/Finding 8's tab-order discussion) -
it never wraps an area's controls in anything with accessibility-group semantics, and the
existing divider `label`s are plain `juce::Label`s with no `AccessibilityRole::group`
association to the controls that follow them. A sighted user infers "these controls belong
together" from column/row proximity and divider text; a screen reader/keyboard user gets a
single flat sequence of controls with no equivalent structure, however clearly the blueprint
author already organized them.
Affected users/failure mode: for any multi-section plugin (e.g. a synth with distinct
oscillator/filter/envelope/effects areas), a screen reader user cannot jump directly to a
section (VoiceOver's rotor/Narrator's landmark navigation has nothing to jump between) and
gets no "entering Filter section" context when tabbing across a section boundary - exactly
the "drilling down" mental model a sighted user gets for free from the visual layout.
Required behavior: each declared area is exposed as a named accessible group; group-to-group
navigation (rotor/landmark-style) reaches any section directly, and entering a group's first
control by Tab announces the group name once.
Recommended mechanism: wrap each `A<n>` block's controls in a lightweight container
`Component` with `AccessibilityRole::group` inside `construct_boxes()`'s existing per-area
FlexBox code (additive, no visual change - the wrapper contains exactly what's already laid
out there); name the group from the area's own divider `label` when one is present as the
first item (reusing `looper.json`'s existing pattern, no new authoring syntax required), or
from an explicit optional per-area title if the schema is later extended (e.g.
`"A2-title": "Filter"` - deferred to Open Questions, not decided here since not every
blueprint has a natural section name for every area).
Scope/dependencies: `parseboxstructure.py`'s `construct_boxes()` (area-to-group wrapping) and
`codegen_widgets.py`. Regression risk: low - purely additive wrapper around already-correct
FlexBox layout code; propagates to all blueprints on regeneration, but visual output is
unchanged (only the accessibility tree gains a layer).
Test: VoiceOver rotor ("Groups") on `looper` (already has divider-labeled sections) - each
section is offered as a direct jump target, and tabbing into it announces the section name
once, not per-control.

**Finding 18 - No drill-down/paged focus scope for blueprints with repeated sub-instance
controls (e.g. a future multi-oscillator synth).**
Files/methods: none in the current codebase touch this yet - closest existing precedent is
the Performance/Settings page-switch (`/*START_PERFORMANCEPAGE*/`, Section 2.3,
`create_page_switch_methods()`) and Finding 16's new Accessibility pane mechanism. Verified
by inspection: none of the 18 current blueprints has a genuinely repeated named sub-instance
control set (e.g. `Osc1*`/`Osc2*`/`Osc3*`) today.
Problem: Finding 17's grouping helps a user *jump to* a section faster but does not reduce
how many controls exist in the flat sequence once there - for a hypothetical repeated-
instance layout (3 oscillators, each with waveform/pitch/level/...), a group landmark at
"Oscillator 2" still leaves every one of Oscillator 1's controls between the start of
navigation and it; reaching "Oscillator 2's waveform" is still every prior instance's full
control count away.
Required behavior: for any future blueprint with genuinely repeated control sets, a true
drill-down interaction - select the sub-instance first (an "Oscillator" selector), which then
scopes visible/focusable controls to only that instance's parameters, collapsing the flat
list from instance-count times per-instance-count controls down to just the latter, at a
time.
Recommended mechanism: not a new interaction model - reuse the existing page-switch machinery
that already toggles component visibility for Performance/Settings, generalized to be driven
by an instance selector rather than a fixed two-way toggle. Pairs naturally with Finding 16's
Accessibility pane, where a drill-down selector is a good fit for a plain, quick-access-
optimized layout specifically (a synth-style multi-oscillator blueprint's Accessibility pane
would present "select oscillator" then that oscillator's flat control list, rather than all
instances' controls at once).
Scope/dependencies: design guidance only - no current blueprint needs this, so no code change
is proposed now; flagged so the pattern is decided in advance of the first blueprint that
needs it (e.g. a future multi-oscillator synth) rather than improvised ad hoc per-blueprint.
Test: not applicable until a repeated-instance blueprint exists; when one does, confirm
...
(Strategy A for nearly everything), with three **Strategy C** exceptions (`StatusBar`,
VU/CPU/Waveform gauges, and the new `DiagnosticsLog`) where the component genuinely has no
natural accessible representation to hang a title/value off of, and four **Strategy E**
exceptions (Spectrogram/CircularBar/SliceWave and the hand-written `customtype` gauges) where
marking the component decorative is the honest choice, per user decision, rather than
inventing a synthetic summary. No component needs a full rewrite (Strategy B). One forward-
looking **Strategy D** item exists (Finding 18, repeated sub-instance controls) but is design
guidance only, since no current blueprint has repeated instances to apply it to - every
existing interaction pattern (drag, text entry, click, right-click menu) is soundly designed;
it's specifically the *accessible surface* (and, for Finding 18, a not-yet-needed navigation
model) around already-correct mouse/keyboard behavior that's missing.
...
| VU/CPU/Waveform gauges | C | `AccessibilityRole::label` + value interface | New minimal `AccessibilityHandler` per class, value read on demand (Finding 4) |
| Diagnostic event log (clip/CPU/silence alerts) | C | `AccessibilityRole::staticText`, live region | New `DiagnosticsLog` component; edge-triggered `notifyAccessibilityEvent` per threshold transition, with hysteresis (Finding 15) |
| Spectrogram/CircularBar/SliceWave, and hand-written `customtype` gauges | E | n/a (`setAccessible(false)`) | Decorative/aesthetic by user decision, no unique safety/task info |
...
| Accessibility pane (new, opt-in) | New page mode, not a per-component strategy | Plain/high-contrast reuse of existing roles | Auto-generated flat control list + minimal `LookAndFeel` variant (Finding 16) |
| Blueprint `A<n>` sections | A | `AccessibilityRole::group` | Wrapper `Component` per area, named from existing divider labels (Finding 17) |
| Repeated sub-instance controls (future) | D (design guidance only) | n/a | Reuse page-switch machinery as an instance selector when a blueprint needs it (Finding 18) |
...
- New Phase-1 observation (not in the original source-review findings): `maxdiffuser`
  ...
  Per user decision (Section 3, Finding 4), these are Strategy E (decorative,
  `setAccessible(false)`) - no further VoiceOver walk needed, no fix pattern to replicate.
...
- Files touched: `inc/CustomRotaryDial.h` (Findings 1, 2, 3), `inc/StatusBar.h`
  (Finding 5), `inc/GenericMeter.h`, `inc/CpuMeter.h`, `inc/WaveformMeter.h` (Findings 4, 15),
  a new `inc/DiagnosticsLog.h` (Finding 15) plus its menu-bar entry point in
  `{Module}Editor.h`, `codegen_widgets.py`'s `create_init_widgets()` (Finding 7).
- New abstractions: `DiagnosticsLog` (Finding 15) - a `StatusBar`-like live-region component
  that logs edge-triggered clip/CPU/silence alerts rather than raw values. Otherwise none new
  - reuses JUCE's own `AccessibilityHandler`/`notifyAccessibilityEvent` API directly in each
  affected class; if duplication across the 3 gauge classes turns out to be significant, a
  small shared free function/mixin in `GenericMeter.h` (the header all three already draw
  shared pieces from) may be introduced, decided during implementation rather than
  pre-declared here.
- Acceptance criteria: on the two Phase-1 canary blueprints, VoiceOver announces a correct
  name for every dial/switch/drop; VU/CPU gauge values are readable on demand without
  continuous chatter; a clip/CPU-overload alert fires exactly once per threshold transition
  in the diagnostics log; status-bar messages are announced without moving focus.
...
- Files touched: `JuceStandaloneGenerator/parseboxstructure.py` (new/reused visual-rank
  helper; also `construct_boxes()`'s area-to-group wrapping for Finding 17),
  `codegen_widgets.py`'s `create_init_widgets()` (Finding 8 - `setExplicitFocusOrder()`
  calls), `inc/LookAndFeel.h` (Finding 9 - explicit focus-ring paint if the default is found
  to be obscured).
- New abstractions: a rank-computation helper reusing `parse_box_structure()`'s existing
  output (no new file); a lightweight `AccessibilityRole::group` wrapper `Component` per
  area (Finding 17, also no new file - added inline in `construct_boxes()`'s generated code).
- Acceptance criteria: Tab/Shift+Tab order on both canary blueprints visually matches the
  ...
  display scaling; on `looper` (has divider-labeled sections), VoiceOver's rotor lists each
  section as a jump target and announces its name once on entry (Finding 17).
...
### Phase 9 - Dedicated accessibility pane (opt-in enhancement, Finding 16)

- Files touched: `codegen_widgets.py` (new auto-layout page-generation function),
  `parseboxstructure.py` (reuse of Finding 8's reading-order helper), a new
  `inc/AccessibilityLookAndFeel.h`, `{Module}Editor.h`'s page-switch codegen splice, blueprint
  JSON schema (new `"accessibility-page"` flag).
- New abstractions: `AccessibilityLookAndFeel` (minimal, stock-shaped, high-contrast) and the
  auto-layout generator (no hand-authored grid, unlike Performance/Settings).
- Work: generalize the existing 2-page Performance/Settings switch to support a third,
  optionally-generated page; wire its toggle into the same page-switch UI already used for
  Performance/Settings; enable on the canary blueprints first.
- Acceptance criteria: on a canary blueprint with `"accessibility-page": true`, every
  parameter is reachable via a single flat Tab sequence with always-visible value readouts,
  high-contrast styling passes the same WCAG check as the Orbit theme suite, and the existing
  main/Performance/Settings pages are unaffected (opt-in, additive).
- Test strategy: `dev-scripts/dev-test-full.sh` build-only check plus a manual keyboard/
  VoiceOver walkthrough of the new pane; `dev-scripts/dev-warnings.sh` clean.
- Risks/rollback: purely additive and opt-in per blueprint, so no risk to existing pages;
  main risk is scope creep into a second full layout system - keep it deliberately simpler
  (flat list only, no grid) rather than mirroring Performance/Settings' composition grid.
- Complexity: **L** (new capability, not a gap fix).
...
| Diagnostic alerts | Drive into/out of clipping and CPU overload; confirm one announcement per transition, not a stream, and that on-demand value queries work independently (Finding 15) | Both canary blueprints |
...
4. **RESOLVED** - Spectrogram/CircularBar/SliceWave, and the hand-written `customtype`
   gauges (Finding 4), are all Strategy E (decorative, `setAccessible(false)`) per user
   decision: the information they carry is aesthetic, not safety- or task-relevant, so no
   per-blueprint check or synthetic accessible value is needed.
...
9. **Default threshold/hysteresis values for Finding 15's diagnostic alerts** - clip level
   (likely 0 dBFS), CPU high-water mark, and sustained-silence duration all need a sensible
   per-metric default and debounce window before implementation; not decided here.
10. **Should Finding 16's accessibility pane be opt-in per blueprint or on by default for
    all 18 once built?** Recommended default here is opt-in (enabled on the canaries first,
    then rolled out) so it can be validated before becoming universal, consistent with how
    Performance/Settings pages were rolled out per-blueprint rather than all at once - not
    a final decision.
11. **Should Finding 17's area groups get an explicit per-area title schema field (e.g.
    `"A2-title": "Filter"`), or should naming rely solely on an existing divider `label`
    being present as the area's first item?** The latter needs no schema change and matches
