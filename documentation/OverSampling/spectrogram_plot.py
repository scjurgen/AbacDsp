...
def main():
    ...
    parser.add_argument("--xrange", type=float, nargs=2, default=None, metavar=("START", "END"),
                         help="replace the default time axis with a linear START->END range "
                              "(e.g. a ratio the data was rendered against); END<START is fine")
    parser.add_argument("--xlabel", type=str, default="time (s)",
                         help="x-axis label; only meaningful together with --xrange")
    ...
    for (path, title), ax in zip(args.panel, axes[:, 0]):
        ...
        if args.xrange is not None:
            ...
            x0, x1 = 0.0, grid.shape[0] * hop / sample_rate
        extent = [x0, x1, 0.0, sample_rate / 2.0]
