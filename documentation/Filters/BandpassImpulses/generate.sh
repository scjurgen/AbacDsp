...
"$BUILD_DIR/documentation/Filters/BandpassImpulses/BandpassImpulseExplore" \
    "$DATA_DIR/rb_response.txt" "$DATA_DIR/rb_decay_accuracy.txt" "$DATA_DIR/rb_compensation_accuracy.txt" \
    "$DATA_DIR/rb_pitchbend.txt" "$DATA_DIR/rb_topology.txt" "$DATA_DIR/rb_isactive.txt"
...
python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/rb_response.txt" -o "$SCRIPT_DIR/rb_response.png" \
    --labelx "frequency (Hz)" --labely "magnitude (dB, normalized to peak)" --width 900 --height 450 --cols 2

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/rb_decay_accuracy.txt" \
    -o "$SCRIPT_DIR/rb_decay_accuracy.png" \
    --labelx "requested 60 dB decay time (s)" --labely "error (%)" --width 900 --height 450 --cols 2

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/rb_compensation_accuracy.txt" \
...
python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/rb_pitchbend.txt" -o "$SCRIPT_DIR/rb_pitchbend.png" \
    --labelx "pitch bend (cents)" --labely "error (cents)" --width 900 --height 450 --cols 2

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/rb_topology.txt" -o "$SCRIPT_DIR/rb_topology.png" \
    --labelx "frequency (Hz)" --labely "magnitude (dB, normalized to peak)" --width 900 --height 450 --cols 2

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/rb_isactive.txt" -o "$SCRIPT_DIR/rb_isactive.png" \
