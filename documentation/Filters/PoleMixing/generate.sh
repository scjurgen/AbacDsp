...
"$BUILD_DIR/documentation/Filters/PoleMixing/PoleMixingExplore" \
    "$DATA_DIR/pm_response.txt" "$DATA_DIR/pm_resonance.txt" "$DATA_DIR/pm_overdrive.txt" \
    "$DATA_DIR/pm_cutoff_accuracy.txt" "$DATA_DIR/pm_realtime.txt" "$DATA_DIR/pm_topology.txt" \
    "$DATA_DIR/pm_raw_correction_data.txt"
...
python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/pm_response.txt" -o "$SCRIPT_DIR/pm_response.png" \
    --labelx "frequency (Hz)" --labely "magnitude (dB) / phase (deg)" --width 1200 --height 450 --cols 1

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/pm_resonance.txt" -o "$SCRIPT_DIR/pm_resonance.png" \
    --labelx "frequency (Hz) / time (s)" --labely "magnitude (dB) / amplitude" --width 1200 --height 350 --cols 1

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/pm_overdrive.txt" -o "$SCRIPT_DIR/pm_overdrive.png" \
    --labelx "input / frequency (Hz)" --labely "output / magnitude (dB)" --width 1200 --height 350 --cols 1

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/pm_cutoff_accuracy.txt" \
...
python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/pm_realtime.txt" -o "$SCRIPT_DIR/pm_realtime.png" \
    --labelx "time (s)" --labely "output amplitude" --width 1200 --height 350 --cols 1

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/pm_topology.txt" -o "$SCRIPT_DIR/pm_topology.png" \
    --labelx "frequency (Hz) / time (s)" --labely "magnitude (dB) / amplitude" --width 1200 --height 350 --cols 1
