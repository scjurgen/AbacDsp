...
"$BUILD_DIR/documentation/OrnsteinUhlenbeck/OrnsteinUhlenbeckTimeline" \
    "$DATA_DIR/ou_timelines.txt" "$DATA_DIR/ou_autocorr.txt" "$DATA_DIR/ou_distribution.txt"
...
python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/ou_timelines.txt" -o "$SCRIPT_DIR/ou_timeline.png" \
    --labelx "time (s)" --labely "x" --width 2400 --height 200 --cols 1 --miny -0.2 --maxy 0.2

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/ou_autocorr.txt" -o "$SCRIPT_DIR/ou_autocorr.png" \
    --labelx "lag tau (s)" --labely "ACF" --width 1200 --height 250 --cols 1 --miny -0.5 --maxy 1.05

python3 "$ROOT_DIR/documentation/Plot/PyConPlot.py" -f "$DATA_DIR/ou_distribution.txt" -o "$SCRIPT_DIR/ou_distribution.png" \
